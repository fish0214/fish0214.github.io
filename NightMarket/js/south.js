function one(){
    document.getElementById('show1').style.display="block";
}
function two(){
    document.getElementById('show2').style.display="block";
}
function thr(){
    document.getElementById('show3').style.display="block";
}
function one1(){
    document.getElementById('show4').style.display="block";
}
function two1(){
    document.getElementById('show5').style.display="block";
}
function thr1(){
    document.getElementById('show6').style.display="block";
}
function one2(){
    document.getElementById('show7').style.display="block";
}
function two2(){
    document.getElementById('show8').style.display="block";
}
function thr2(){
    document.getElementById('show9').style.display="block";
}
function heart1(){
    document.getElementById('love1').style.display="block";
}
function heart2(){
    document.getElementById('love2').style.display="block";
}
function heart3(){
    document.getElementById('love3').style.display="block";
}
function heart4(){
    document.getElementById('love4').style.display="block";
}
function heart5(){
    document.getElementById('love5').style.display="block";
}
function heart6(){
    document.getElementById('love6').style.display="block";
}
function heart7(){
    document.getElementById('love7').style.display="block";
}
function heart8(){
    document.getElementById('love8').style.display="block";
}
function heart9(){
    document.getElementById('love9').style.display="block";
}
  function CloseModal() {
    let element = document.getElementById('popup')
    element.style.display = 'none'
  }